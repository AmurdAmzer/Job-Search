...
async function searchJobs(queryText) {
  const formattedQuery = queryText
    .split('\n')
    .map(line => line.replace('-', '').trim())  // 👈 no more `.filter()`
    .join(', ');

  console.log("🔍 Final query to JSearch:", formattedQuery);
...
app.get('/api/search-jobs', async (req, res) => {
  console.log("🔍 Received GET /api/search-jobs request:", req.query);
  ...
  try {
    ...
    console.error("❌ JSearch call failed:", err.message);
    res.status(500).json({ message: 'Failed to fetch jobs', error: err.message });
  }
});
